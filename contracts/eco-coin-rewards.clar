...
(define-map pending-claims (tuple user principal center principal) { amount: uint, claim-id: uint })
...
(define-private (is-owner) (is-eq tx-sender CONTRACT_OWNER))
(define-private (is-center (center principal)) (is-some (map-get? certified-recycling-centers center)))

;; --- Administrative Functions ---

;; Add a new certified recycling center.
;; @param center_principal: The principal of the recycling center.
;; @param center_name: The name of the center.
(define-public (add-center (center_principal principal) (center_name (string-ascii 50)))
  (begin
    (asserts! (is-owner) ERR_UNAUTHORIZED)
    (map-set certified-recycling-centers center_principal { name: center_name })
    (ok true)
  )
)

;; Remove a certified recycling center.
;; @param center_principal: The principal of the center to remove.
(define-public (remove-center (center_principal principal))
  (begin
    (asserts! (is-owner) ERR_UNAUTHORIZED)
    (map-delete certified-recycling-centers center_principal)
    (ok true)
  )
)
...
(define-public (redeem-for-stx (amount uint))
  (begin
    ...
    (let ((stx-value (/ (* amount u10000) u1))) ;; a simple rate of 10000 uSTX per ECO
...
(define-read-only (get-name) (ok "Eco-Coin"))
(define-read-only (get-symbol) (ok "ECO"))
(define-read-only (get-decimals) (ok u6))
(define-read-only (get-balance (who principal)) (ok (ft-get-balance eco-coin who)))
(define-read-only (get-total-supply) (ok (ft-get-total-supply eco-coin)))
(define-read-only (get-token-uri) (ok (var-get token-uri)))
(define-read-only (get-treasury-balance) (ok (var-get treasury-balance)))
(define-read-only (is-claim-processed (claim-id uint)) (default-to false (map-get? processed-claims claim-id)))
(define-read-only (get-pending-claim (user principal) (center principal)) (map-get? pending-claims {user: user, center: center}))
(define-read-only (get-center-info (center principal)) (map-get? certified-recycling-centers center))